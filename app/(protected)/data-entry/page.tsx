        ...
        <form onSubmit={submitTraffic} className="space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <Field label="Ad Spend (Monat)">
              <input type="number" value={adSpend} onChange={(e) => setAdSpend(e.target.value)} className="w-full bg-white border border-neutral-300 rounded px-3 py-2" />
            </Field>
            <Field label="Lead Eingang gesamt">
              <input type="number" value={leads} onChange={(e) => setLeads(e.target.value)} className="w-full bg-white border border-neutral-300 rounded px-3 py-2" />
            </Field>
            <Field label="Terminiert gesamt">
              <input type="number" value={scheduled} onChange={(e) => setScheduled(e.target.value)} className="w-full bg-white border border-neutral-300 rounded px-3 py-2" />
            </Field>
            <Field label="Verloren in Terminierung">
              <input type="number" value={lostScheduling} onChange={(e) => setLostScheduling(e.target.value)} className="w-full bg-white border border-neutral-300 rounded px-3 py-2" />
            </Field>
          </div>
        ...
        <form onSubmit={submitCloser} className="space-y-3">
          ...
          {attendance === 'ERSCHIENEN' && outcome === 'FOLLOW_UP' ? (
            <Field label="Follow-Up Termin">
              <input type="date" value={followUpDate} onChange={(e) => setFollowUpDate(e.target.value)} className="w-full bg-white border border-neutral-300 rounded px-3 py-2" />
            </Field>
          ) : null}
          ...
          {attendance === 'ERSCHIENEN' && outcome === 'CLOSED' ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <Field label="Gesamthöhe Abschluss (€)">
                <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} className="w-full bg-white border border-neutral-300 rounded px-3 py-2" />
              </Field>
              {paymentType === 'INSTALLMENT' ? (
                <>
                  <Field label="Ratenhöhe (€)">
                    <input type="number" value={installmentAmount} onChange={(e) => setInstallmentAmount(e.target.value)} className="w-full bg-white border border-neutral-300 rounded px-3 py-2" />
                  </Field>
                  <Field label="Anzahl Raten">
                    <input type="number" value={installmentCount} onChange={(e) => setInstallmentCount(e.target.value)} className="w-full bg-white border border-neutral-300 rounded px-3 py-2" />
                  </Field>
                </>
              ) : null}
            </div>
...
function Select({ label, value, onChange, options }: { label: string; value: string; onChange: (v: string) => void; options: Array<{ v: string; l: string }> }) {
  return (
    <label className="text-sm block">
      {label}
      <select className="w-full bg-white border border-neutral-300 rounded px-3 py-2" value={value} onChange={(e) => onChange(e.target.value)}>
